...
function einlaufListeForm($html) {
	...
	foreach ($result[0] as $row) {
		...
		$html2 .= "<td width=\"100\" align=\"left\">".$row['start']."</td>\n";
...
function showEinlaufListe($rennen, $action) {
	...
	if($_SESSION['rennenIDs'] != "") {
		...
		$sql = "update $tmptable set manzeit = zielzeit where zielzeit is not NULL and usemantime = 0";
		$result = dbRequest($sql, 'UPDATE');
		
		$sql = "SELECT * from $tmptable where (zielzeit is not NULL or usemantime = 2) and usemantime <> 1 order by manzeit, millisecond asc;";
		$result = dbRequest($sql, 'SELECT');
		...
		if($result[1] > 0) {
			foreach ($result[0] as $row) {
				...
				if ($row['usemantime'] == 2 ) {
				$html2 .= "&nbsp;&nbsp;|&nbsp;&nbsp;<a class=\"manzeit\" id=\"".$row['ID']."\" href=\"jqRequest&func=saveManZielzeit&id=".$row['ID']."&action=del\"><i class=\"fa fa-times fa-lg\"></i></a></span></td>\n";
				}
